/* Virtuino bluetooth library ver 1.4.4
...
void  VirtuinoBluetooth::run() {
  ...
  if (BTserial->available()) readBluetoothSerialData2();    // while serial buffer is not empty we read and store the character to commandBuffer
...
//This is a system fuction. Don't change this code
 ...
 void VirtuinoBluetooth::readBluetoothSerialData(){
  ...
  void VirtuinoBluetooth::readBluetoothSerialData2(){
    ...
    switch (activeCommandType) {
      case 'I':                           //digital output command = +Q05=0000#   or  = +Q13=0001#
            if ((activeCommandPin>=0) & (activeCommandPin < arduinoPinsSize))
              response =COMMAND_START_CHAR+String(activeCommandType)+pinString+"="+String(digitalRead(activeCommandPin))+COMMAND_END_CHAR;  // response 
            else   response =getErrorCommand(ERROR_PIN);  // response  error pin number   !E00=1$       
          break;
      case 'Q': 
            if ((activeCommandPin>=0) & (activeCommandPin < arduinoPinsSize)){
                 if (returnInfo) response =COMMAND_START_CHAR+String(activeCommandType)+pinString+"="+String(digitalRead(activeCommandPin))+COMMAND_END_CHAR;  // response 
                 else 
           ...
           case 'C':                   
            if ((activeCommandPin>=0) & (activeCommandPin <=1)){          //0 connect via bluetooth - 1 connect via network 
               if (activeCommandValue==1) {
                 ...
                 response ="!C"+pinString+"="+String(firmwareCode)+"$";
               }
              else if (activeCommandValue==0) {
                connectedStatus=false;
                response =COMMAND_START_CHAR+"C"+pinString+"=0"+COMMAND_END_CHAR;
               }
              else      response =getErrorCommand(ERROR_VALUE);;
              
            } else   response =getErrorCommand(ERROR_COMMAND);        
          break;  
           }
